...
const tools = [
      ...
      {name:'Shodan', category:'OSINT', description:'Search engine for internet-connected devices.', tags:['osint','iot'], risk:'low', logo:'https://cdn.simpleicons.org/shodan', url:'https://www.shodan.io', commands:['Use Shodan web UI or API']},
...
CATEGORIES.forEach(cat=>{
  ...
  btn.textContent = cat;
  btn.onclick = ()=>{ activeCategory = cat; document.querySelectorAll('.tabs button').forEach(b=>b.classList.remove('bg-slate-700','text-white')); btn.classList.add('bg-slate-700','text-white'); renderToolList(searchInput.value||''); };
...
function renderToolList(filter=''){
  ...
  tools.forEach((t, idx)=>{
    ...
    div.innerHTML = `
      <div class="w-10 h-10 rounded-md bg-slate-800 flex items-center justify-center text-sm font-medium"><img src="${t.logo||''}" alt="" class="w-8 h-8 object-contain" onerror="this.style.display='none'"/></div>
      <div class="flex-1">
...
function selectTool(idx){
  ...
  toolTags.innerHTML = '';
  (t.tags||[]).forEach(tag=>{ const span = document.createElement('span'); span.className='text-xs px-2 py-1 rounded-md bg-slate-800 border border-slate-700'; span.textContent=tag; toolTags.appendChild(span); });
